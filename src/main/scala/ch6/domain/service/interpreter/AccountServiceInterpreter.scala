...
class AccountServiceInterpreter
  ...
  override def close(no: String,
                     closeDate: Option[Date]): AccountOperation[Account] = kleisli[Valid, AccountRepository, Account]{
    (repo: AccountRepository) =>
    repo.query(no) match {
  ...
  override def balance(no: String) = kleisli[Valid, AccountRepository, Balance] { (repo: AccountRepository) => repo.balance(no) }
}
