...
object AllGen {
  ...
  def genOptionvalValidCloseDate(seed: Date) =
    Gen.frequency(
      (8, Some(aDateAfter(seed))),
      (1, None)
    )
  def aDateAfter(date: Date) = new Date(date.getTime + 100000)
  def aDateBefore(date: Date) = new Date(date.getTime - 100000)
...
object CheckingAccountSpecification extends Properties("Account") {
  ...
  val validCheckingAccountGen = for {
    ...
    nm <- genName
    od <- arbitrary[Date]
    cd <- genOptionvalValidCloseDate(od)
  ...
  val validClosedCheckingAccountGen = for {
    ...
    nm <- genName
    od <- arbitrary[Date]
    cd <- genOptionvalValidCloseDate(od) suchThat(_ isDefined)
    bl <- arbitrary[Balance]
  } yield checkingAccount(no, nm, Some(od), cd, bl)
